    ...
    const fetchData = async (token) => {
        try {
            ...
            const response = await axios.get("/api/users", {
                headers: { Authorization: `Bearer ${token}` }, // Include the JWT token in the request header
            });
            
            // Store the received user data in state
            setData(response.data);
        } catch (err) {
            setError("Failed to fetch data"); // Show an error message if fetching data fails
        }
    };
